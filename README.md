<div align="center">

# [TienNM99](https://tiennm99.github.io)

</div>

## About me

> Just a developer who wants to be a magician

I am a passionate developer who believes in the magic of code. I enjoy turning complex problems into elegant solutions and bringing ideas to life through the art of programming.

My favorite username — `miti99` — is a creative fusion of my Vietnamese name "Nguyễn Minh Tiến" and my birth year, 1999[^username]. It represents both my identity and my journey in the tech world.

## Links

<div align="center">

[![Blog](https://custom-icon-badges.demolab.com/badge/Blog-f48120?style=for-the-badge&logoSource=feathe&logo=home&logoColor=white)](https://tiennm99.github.io/)
[![Pdf CV](https://custom-icon-badges.demolab.com/badge/Pdf_CV-f40f02?style=for-the-badge&logoSource=feather&logo=file-text&logoColor=white)](https://tiennm99.github.io/cv/miti99.pdf)
[![Web CV](https://custom-icon-badges.demolab.com/badge/Web_CV-211f1f?style=for-the-badge&logoSource=feather&logo=github&logoColor=white)](https://tiennm99.github.io/cv)
[^badge]

</div>

## Contact

<div align="center">

[![Outlook](https://custom-icon-badges.demolab.com/badge/Outlook-0072c6?style=for-the-badge&logoSource=feather&logo=mail&logoColor=white)](mailto:tiennm99@outlook.com)
[![Facebook](https://custom-icon-badges.demolab.com/badge/Facebook-1778f2?style=for-the-badge&logoSource=feather&logo=facebook&logoColor=white)](https://www.facebook.com/tiennm99)
[![LinkedIn](https://custom-icon-badges.demolab.com/badge/LinkedIn-006192?style=for-the-badge&logoSource=feather&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/miti99/)
[![Telegram](https://custom-icon-badges.demolab.com/badge/Telegram-0088cc?style=for-the-badge&logoSource=feather&logo=telegram&logoColor=white)](https://t.me/tiennm5)

</div>

## Stats

<div align="center">

<table>
<tr>
<td><img src="./output/dracula/profile-details.svg" alt="profile" /></td>
<td><img src="./output/dracula/stats.svg" alt="stats" /></td>
</tr>
<tr>
<td><img src="./output/dracula/streak.svg" alt="streak" /></td>
<td><img src="./output/dracula/top-starred-repos.svg" alt="top starred repos" /></td>
</tr>
<tr>
<td><img src="./output/dracula/repos-per-language.svg" alt="repos per language" /></td>
<td><img src="./output/dracula/contributions-heatmap.svg" alt="contributions heatmap" /></td>
</tr>
<tr>
<td colspan="2" align="center"><img src="./output/dracula/contributions-by-year.svg" alt="contributions by year" /></td>
</tr>
<tr><th>Last year</th><th>All time</th></tr>
<tr>
<td><img src="./output/dracula/most-commit-language.svg" alt="most commit language last year" /></td>
<td><img src="./output/dracula/most-commit-language-all-time.svg" alt="most commit language all time" /></td>
</tr>
<tr>
<td><img src="./output/dracula/productive-time.svg" alt="productive time last year" /></td>
<td><img src="./output/dracula/productive-time-all-time.svg" alt="productive time all time" /></td>
</tr>
<tr>
<td><img src="./output/dracula/productive-weekday.svg" alt="productive weekday last year" /></td>
<td><img src="./output/dracula/productive-weekday-all-time.svg" alt="productive weekday all time" /></td>
</tr>
<tr>
<td><img src="./output/dracula/contributions.svg" alt="contributions last year" /></td>
<td><img src="./output/dracula/contributions-all-time.svg" alt="contributions all time" /></td>
</tr>
</table>

</div>

<div align="right">

_Generated by [ghstats](https://github.com/tiennm99/ghstats)_

</div>

[^username]: When `miti99` is unavailable (like on GitHub), I often use `tiennm99`, which incorporates parts of my family name, middle name, given name, and birth year.

    Another common username is `tienthieusac` (_Tiên Thiếu Sắc_), meaning _Tiên_ is _Tiến_ without the accent[^tienthieusac].

    If none of these are available, I may use other minor usernames, although I do not keep track of all of them.

[^tienthieusac]: Inspired by [Nguyễn Tuân](https://vi.wikipedia.org/wiki/Nguy%E1%BB%85n_Tu%C3%A2n)'s pen name: _Tuấn Thừa Sắc_, which represents how _Tuân_ becomes _Tuấn_ with an optional accent.

[^badge]: Badges for Blog, PDF CV, and Web CV reference their respective platforms — Cloudflare, Adobe, and GitHub. Badge colors are referenced from [SchemeColor](https://www.schemecolor.com/).
